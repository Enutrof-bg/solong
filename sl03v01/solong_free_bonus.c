/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   solong_free_bonus.c                                :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: kevwang <kevwang@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/10 11:46:03 by kevwang           #+#    #+#             */
/*   Updated: 2025/06/10 11:46:05 by kevwang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "solong.h"

void	ft_destroy(t_data *data)
{
	if (data->anim.img_anim1)
		mlx_destroy_image(data->mlx, data->anim.img_anim1);
	if (data->anim.img_anim2)
		mlx_destroy_image(data->mlx, data->anim.img_anim2);
	if (data->anim.img_anim3)
		mlx_destroy_image(data->mlx, data->anim.img_anim3);
	if (data->anim.img_anim4)
		mlx_destroy_image(data->mlx, data->anim.img_anim4);
	if (data->anim.img_key_anim1)
		mlx_destroy_image(data->mlx, data->anim.img_key_anim1);
	if (data->anim.img_key_anim2)
		mlx_destroy_image(data->mlx, data->anim.img_key_anim2);
	if (data->anim.img_key_anim3)
		mlx_destroy_image(data->mlx, data->anim.img_key_anim3);
	if (data->anim.img_key_anim4)
		mlx_destroy_image(data->mlx, data->anim.img_key_anim4);
}

int	on_destroy(t_data *data)
{
	if (data->img.img_grass)
		mlx_destroy_image(data->mlx, data->img.img_grass);
	if (data->img.img_wall)
		mlx_destroy_image(data->mlx, data->img.img_wall);
	if (data->img.img_char)
		mlx_destroy_image(data->mlx, data->img.img_char);
	if (data->img.img_key)
		mlx_destroy_image(data->mlx, data->img.img_key);
	if (data->img.img_exit)
		mlx_destroy_image(data->mlx, data->img.img_exit);
	if (data->img.img_exit_open)
		mlx_destroy_image(data->mlx, data->img.img_exit_open);
	if (data->img.img_ennemy)
		mlx_destroy_image(data->mlx, data->img.img_ennemy);
	if (data->img.img_ennemy_mort)
		mlx_destroy_image(data->mlx, data->img.img_ennemy_mort);
	ft_destroy(data);
	mlx_destroy_window(data->mlx, data->mlx_win);
	mlx_destroy_display(data->mlx);
	if (data->map)
		ft_free_double_tab(data->map);
	free(data->mlx);
	exit(0);
	return (0);
}
