...
void ft_check_dead(t_data *data)
{
	if (data->player_x == data->ennemy_x && data->player_y == data->ennemy_y)
		on_destroy(data);
	if (data->player_x == data->ennemy_mort_x && data->player_y == data->ennemy_mort_y)
		on_destroy(data);
}
...
void ft_set_ennemy_mort(t_data *data)
{
	...
	print_img_ennemy_mort(data, &data->img, data->ennemy_mort_y, data->ennemy_mort_x);
}
...
void ft_ennemy_move(t_data *data)
{
	...
	if (move >= 10)
		move = 0;
	ft_printf("n:px%d py%d ex%d ey%d \n", data->player_x, data->player_y, data->ennemy_x, data->ennemy_y);
	if (move < 5)
	{
		if ((data->ennemy_y != data->player_y) || (data->ennemy_x != data->player_x))
...
void ft_ennemy_move_mort(t_data *data)
{
	...
	if (move >= 10)
		move = 0;
	ft_printf("m:px%d py%d ex%d ey%d \n", data->player_x, data->player_y, data->ennemy_mort_x, data->ennemy_mort_y);
	if (move < 5)
	{
		if ((data->ennemy_mort_y != data->player_y) || (data->ennemy_mort_x != data->player_x))
			print_img_grass(data, &data->img, data->ennemy_mort_y, data->ennemy_mort_x);
		print_img_ennemy_mort(data, &data->img, data->ennemy_mort_y, data->ennemy_mort_x + 1);
		data->ennemy_mort_x++;
		move++;
	}
	else if (move < 10)
...
// int	ft_move_right_mort(t_data *data)
// {
